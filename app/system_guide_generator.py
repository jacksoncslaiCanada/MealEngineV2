...
def _image_to_data_uri(url: str) -> str:
    """Fetch image from URL and return base64 data URI. Returns empty string on failure."""
    import base64
    import httpx
    try:
...
def _generate_recipe_image(name: str, description: str) -> str:
    """Generate a 16:9 food photo via Flux Schnell and return a base64 data URI."""
    import base64
    import httpx
    import time
    ...
    try:
        ...
        prompt = (
            f"Professional food photography, wide editorial overhead shot. {name}. "
            f"{first_sentence}. White marble countertop, natural window light from the left, "
            "shallow depth of field, food styling. Clean, editorial. Bon Appetit magazine quality."
        )
        ...
        logger.warning("system_guide_generator: image generation failed for %s — %s", name, exc)
        return ""


def _resolve_image(supabase_url: str, name: str, description: str) -> str:
    """Return a base64 data URI for the image.

    Strategy:
      1. Try fetching the pre-generated image from Supabase (fast path).
      2. If missing, call Flux Schnell to generate one on the fly.
      3. Return empty string if both fail (placeholder renders in HTML).
    """
    # Fast path: image already generated by cron
    uri = _image_to_data_uri(supabase_url)
    if uri:
        logger.debug("system_guide_generator: loaded pre-generated image from %s", supabase_url)
        return uri

    # Slow path: generate now
    logger.info("system_guide_generator: no image at %s — generating via Flux", supabase_url)
...
def generate_system_guide_pdf(slug: str, *, cover_image_url: str | None = None) -> bytes:
    ...
    with ThreadPoolExecutor(max_workers=6) as pool:
        ...
        ) if (cover_image_url is None and cover_supabase_url) else None
    ...
    html = template.render(**guide_data, cover_image_url=cover_image_url)
